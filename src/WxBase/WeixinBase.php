...
use birksCha\wxbase\WeixinTool;


class WeixinBase extends WeixinTool{
    ...
    public function get_access_token($appid='',$appsecret=''){
        if(empty($appid) || empty($appsecret)){
            $url = 'https://api.weixin.qq.com/cgi-bin/token?grant_type=client_credential&appid='.$this->base['appId'].'&secret='.$this->base['appSecret'];
        }else{
            $url = 'https://api.weixin.qq.com/cgi-bin/token?grant_type=client_credential&appid='.$appid.'&secret='.$appsecret;
        }
        return $this->curl_data($url);
    }
...
//    }
	...
	public function send_url($param=[]){
        if(empty($param['appId']) || empty($param['appSecret'])){
            $url = 'https://api.weixin.qq.com/sns/jscode2session?appid='.$this->base['appId'].'&secret='.$this->base['appSecret'].'&js_code='.$param['code'].'&grant_type=authorization_code';
        }else{
            $url = 'https://api.weixin.qq.com/sns/jscode2session?appid='.$param['appId'].'&secret='.$param['appSecret'].'&js_code='.$param['code'].'&grant_type=authorization_code';
        }
		return $this->curl_data($url,$param);
	}
...
//        $tokenInfo = json_decode($access_token,true);
//        //判断access_token是否失效
//        if($tokenInfo['expires_in'] - time() < 300){
//            //重新获取access_token
...
//        $tokenInfo = $this->set_token()['access_token'];
        $tokenInfo = $param['access_token'];
        //获取二维码
//        $url = 'https://api.weixin.qq.com/cgi-bin/wxaapp/createwxaqrcode?access_token='.$tokenInfo; //方式C
//        $url = 'https://api.weixin.qq.com/wxa/getwxacode?access_token='.$tokenInfo; //方式A
        $url = 'https://api.weixin.qq.com/wxa/getwxacodeunlimit?access_token='.$tokenInfo; //方式B
